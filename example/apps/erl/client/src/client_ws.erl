...
websocket_handle({binary, <<ReplyJobId:?ID_LENGTH>>}, _ConnState, #ws_client_ctx{hostname=Hostname}=Ctx) ->
...
websocket_info(trigger, _ConnState, #ws_client_ctx{job_cnt=JobCnt, hostname=Hostname, delay=Delay}=Ctx) ->
  ...
  {reply, {binary, <<JobCnt:?ID_LENGTH>>}, Ctx#ws_client_ctx{job_cnt=JobCnt+1}};
